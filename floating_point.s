...
main:   
        ...
        # 128: 74.43233
        # 200: 1825.4126
        addi     a2, x0, 100            # change the immediate for a different array size
        lui      a0, 0x10010
        addi     a1, zero, 32
        add      a1, a0, a1
        jal      ra, dot_product
...
dot_product:
        ...
        # a0 is the address of array of floats in x
        # a1 is the address of array of floats in y
        # a2 is the integer 
  
        addi s0, x0, 0		# s0 = 0; i = 0 for the loop
        addi t0, x0, 0		# t0 = 0; sum = 0
        fcvt.s.w fa0, t0	# fa0 = 0.0 
        
        beq x0, x0, test	# Go to test
loop:
	slli t1, s0, 2		# t1 = 4(s0); Offset 4i 
	add t2, a0, t1		# Calculate address of x[i]
	add t3, a1, t1		# Calculate address of y[i]
	
	flw ft0, 0(t2)		# ft0 is the float at x[i]
	flw ft1, 0(t3)		# ft1 is the float at y[i]
...
test:
	blt s0, a2, loop	# If s0 < a2 keep looping; If i < n keep looping  
